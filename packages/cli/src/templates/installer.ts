...
export async function installFrontendTemplate(
...
} from "../utils/contract-resolver.js";
...
export async function installContracts(
  ...
  try {
    ...
    for (const contractId of contractsToInstall) {
      const templatePath = CONTRACT_MAP[contractId];
      if (!templatePath) continue;

      const [dir, name] = templatePath.split("/");
      const contractTemplateDir = path.join(templatesDir, "contracts", dir);

      // Copy contract file
      const contractFile = path.join(contractTemplateDir, `${name}.clar`);
      if (await fs.pathExists(contractFile)) {
        await fs.copy(contractFile, path.join(contractsPath, `${name}.clar`));
      }

      // Copy test file
      const testFile = path.join(contractTemplateDir, `${name}.test.ts`);
      if (await fs.pathExists(testFile)) {
        await fs.copy(testFile, path.join(testsPath, `${name}.test.ts`));
      }
    }
