...
export async function deployCommand(
  ...
  if (!(await fs.pathExists(path.join(projectPath, "Clarinet.toml")))) {
    console.error(
      chalk.red("Error: Not in a create-stacks-app project directory."),
    );
    process.exit(1);
  }
    ...
    const { confirm } = await inquirer.prompt([
      {
        ...
        message: chalk.yellow(
          "⚠️  You are about to deploy to MAINNET. This is irreversible. Continue?",
        ),
        default: false,
      },
    ]);
  ...
  try {
    ...
    try {
      ...
      console.error(
        "Please install Clarinet: https://github.com/hirosystems/clarinet",
      );
      process.exit(1);
    }
    ...
    const deploymentPlan = await createDeploymentPlan(
      projectPath,
      network,
      contracts,
    );
    ...
    try {
      ...
      console.log(
        `  1. Update ${chalk.cyan("NEXT_PUBLIC_CONTRACT_ADDRESS")} in your frontend .env`,
      );
      console.log(
        `  2. Verify contracts on explorer: ${getExplorerUrl(network)}`,
      );
      console.log(
        `  3. Test your dApp at ${chalk.cyan("http://localhost:3000")}`,
      );
    } catch (error: unknown) {
      ...
      if ((error as Error).message.includes("insufficient funds")) {
        ...
        console.log(
          chalk.yellow(
            "Tip: Make sure you have enough STX for deployment fees.",
          ),
        );
        if (network === "testnet") {
          console.log(
            `Get testnet STX from the faucet: ${chalk.cyan("https://explorer.stacks.co/sandbox/faucet?chain=testnet")}`,
          );
        }
      }
